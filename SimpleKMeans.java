...
public class SimpleKMeans 
  ...
  public void buildClusterer(Instances data) throws Exception {
    ...
    {//multithread
      final List threadList = new ArrayList( m_NumThreads );
      final Instances _instances = instances;
    ...
    if( ( i % m_NumThreads ) != ti ) {
      continue;    
    }
      m_FullMissingCounts[i] = _instances.attributeStats(i).missingCount;
      if (_instances.attribute(i).isNumeric()) {
        if (m_displayStdDevs) {
          m_FullStdDevs[i] = Math.sqrt(_instances.variance(i));
        }
        if (m_FullMissingCounts[i] == _instances.numInstances()) {
          m_FullMeansOrMediansOrModes[i] = Double.NaN; // mark missing as mean
        }
      } else {
        m_FullNominalCounts[i] = _instances.attributeStats(i).nominalCounts;
      ...
      for( int l = 0; l < threadList.size(); ++l ) {
        ( ( Thread )threadList.get( l ) ).join();
      }
    
    }//multithread
    
    m_ClusterCentroids = new Instances(instances, m_NumClusters);
    final int[] clusterAssignments = new int [instances.numInstances()];
    ...
    while (!converged) {
      ...
      { //multithread
      final List threadList = new ArrayList( m_NumThreads );
      final Instances fInstances = instances;
      final boolean[] convergedArray = new boolean[]{ true };
      final int numThreads = m_NumThreads;
      for( int threadIndex = 0; threadIndex < m_NumThreads; ++threadIndex ) {
        final int ti = threadIndex;
        final Thread thread = new Thread() {
        public void run() {
        ...
        //thread run() -->
        for (int j = 0; j < fInstances.numInstances(); j++) {
          if( ( j % numThreads ) != ti ) {
            continue;
          }
	Instance toCluster = fInstances.instance(j);
	int newC = clusterProcessedInstance(toCluster, true);
	if (newC != clusterAssignments[j]) {
	  synchronized( convergedArray ) {
	    convergedArray[ 0 ] = false;
	  }
	}
	clusterAssignments[j] = newC;
        }
        //thread run() <--
	...
	tempI[clusterAssignments[i]].add(instances.instance(i));
      ...
      { //multithread
      final Map indexNewCentroidMap = Collections.synchronizedMap( new HashMap() );
      final List threadList = new ArrayList();
      final int numThreads = m_NumThreads;
      final int[] emptyClusterCountArray = new int[]{ 0 };
      
      for( int threadIndex = 0; threadIndex < m_NumThreads; ++threadIndex ) {
        final int ti = threadIndex;
        final Instances[] tempI2 = tempI;
        final Thread thread = new Thread() {
        public void run() {
	  ...
	  if (tempI2[j].numInstances() == 0) {
	    // empty cluster
	    synchronized( emptyClusterCountArray ) {
	      emptyClusterCountArray[ 0 ] ++;
	    }
	  } else {
	    final double[] newCentroid = moveCentroid( j, tempI2[j], false  );
	    indexNewCentroidMap.put( new Integer( j ), newCentroid );
	  }
        }
      ...
      for( int l = 0; l < m_NumClusters; ++l ) {
        final Object key = new Integer( l );
        double[] newCentroid = ( double[] )indexNewCentroidMap.get( key );
        if( newCentroid != null ) {
          m_ClusterCentroids.add( new Instance(1.0, newCentroid));
        }
      }
      emptyClusterCount = emptyClusterCountArray[ 0 ];
      } //multithread
   ...
   */
  public void setNumThreads(int n) throws Exception {
  if( n <= 0 ) {
    throw new Exception("Number of threads must be > 0");
  }
    m_NumThreads = n;
  }
